...
void routing_manager_impl::subscribe(
    ...
    //work around solution to send delayed(10ms) subscribe request as cp2 does n't handle/respond immediate subscribe requests from hpc(a53) VM's
    std::this_thread::sleep_for(std::chrono::milliseconds(SD_SUBSCRIBE_REQUEST_DELAY));
    VSOMEIP_INFO << "SUBSCRIBE WITH DELAY("
        << std::hex << std::setfill('0')
        << std::setw(4) << _client << "): ["
        << std::setw(4) << _service << "."
        << std::setw(4) << _instance << "."
...
void routing_manager_impl::reduce_msg_length(byte_t *buffer, length_t& bufferSize,
    ...
    length_t its_length = VSOMEIP_BYTES_TO_LONG(
        buffer[VSOMEIP_LENGTH_POS_MIN], buffer[VSOMEIP_LENGTH_POS_MIN + 1],
        buffer[VSOMEIP_LENGTH_POS_MIN + 2], buffer[VSOMEIP_LENGTH_POS_MAX]);
...
bool routing_manager_impl::apply_e2e_protection(const byte_t **_data, length_t &_size,
    ...
    length_t payload_length = VSOMEIP_WORDS_TO_LONG(
            (*_data)[VSOMEIP_LENGTH_POS_MIN], (*_data)[VSOMEIP_LENGTH_POS_MAX]);
